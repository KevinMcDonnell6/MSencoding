...
with tf.Session(config=config) as session:
    ...
    for i in range(int(np.ceil(len(test_spec)/model_utils.test_stack_size))):
        test_data = list(
                          zip(*multiprocessing_pool.map(dp.process_spectrum_Graph,
        ...
        for start in range(0, n_dev, dev_batch_size):
            ...
            AA_windows = np.array(test_data[2])[indices]
            
            AA_windows_bw = np.array(test_data[3])[indices]
            
            features = np.array(test_data[4])[indices]
            
            ion_windows = np.array(test_data[5])[indices]
            
            indx, vals = list(zip(*[test_data[6][idx] for idx in indices])) 
