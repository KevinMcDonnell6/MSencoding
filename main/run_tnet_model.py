...
model_utils.train_batch_size = 1
...
def tf_run_data(model,
    ...
    for start in range(0, stack_size, batch_size):
        ...
        ion_loc = np.array(data[2])[indices]
        ion_loc_bw = np.array(data[3])[indices]
        
        peaks = np.array(data[4])[indices]
        
        features = np.array(data[5])[indices]
        
        ion_loc_self = np.array(data[6])[indices]
...
def train_epoch(model,
    ...
    #                   VAL PHASE
        ...
        if i%val_period==0:
            val_data = list(
                              zip(*MP.map(dp.process_spectrum_Tnet,
